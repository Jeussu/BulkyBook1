...
| `SMOKE-07` | P0 | `/Customer/Cart/Summary` | Individual | Submit individual checkout using either valid Stripe test keys or the explicit `Stripe__EnableLocalCheckoutFallback=true` demo/staging path. | With valid Stripe keys, Stripe checkout redirects and confirms normally. With missing/invalid Stripe keys and fallback enabled, a no-payment demo order is created, total is server-recomputed, stock decreases, cart clears, and the cart badge becomes `0`. With fallback disabled and Stripe missing, checkout blocks cleanly without creating a bad order. | Order confirmation/error screenshot plus latest order/cart/stock DB query. |  |  |
